...
def access_pixels(img):
    ...
    oldPixelColor = 0
    # for row in range(height,0,-100):    #遍历每一行
        # for col in range(width,width - 30,-1): #遍历每一列

    for row in range(0,heigth,1000):    #遍历每一行
        for col in range(0,width,1000): #遍历每一列
...
row,col = access_pixels(src)         #访问图像的每个元素并处理
print(row,col)
