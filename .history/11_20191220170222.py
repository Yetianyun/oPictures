...
def access_pixels(img):
    ...
    for row in range(0,heigth,100):    #遍历每一行
        for col in range(0,width,200): #遍历每一列
            ...
            for channel in range(channels):    #遍历每个通道（三个通道分别是BGR）
                ...
                minColor = pixelColor - oldPixelColor
                if ( minColor < 0 )
                    minColor = pixelColor * ( -1)
            if ( pixelColor > 100000000) 
                widthLine = abs(oldPixelColor - pixelColor) 
